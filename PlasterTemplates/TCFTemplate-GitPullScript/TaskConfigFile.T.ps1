...
#>
@{
    TaskName    = 'Github - Pull <%=$($PLASTER_PARAM_RepoName)%>'
    TaskPath    = '\Github Core\'
    Description = 'Clones the <%=$($PLASTER_PARAM_RepoName)%> Repo'
    TaskAction  = @{
        Execute                         = 'powershell.exe' 
        Argument                        = "-WindowStyle Hidden -File 'D:\GitHub\De-Automation\ServerScripts\Git-<%=$PLASTER_PARAM_RepoName%>-Repo.ps1"
        Id                              = '<%=$(New-GUID)%>'
        WorkingDirectory                = 'D:\'
    }
    ...
    PrincipalSecretName                 = 'USSDesktopEng'
